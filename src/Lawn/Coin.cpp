...
void Coin::Update()
{
	...
	// Auto-collect: pick up coins and sun automatically instead of waiting for a click.
	if (!mDead && !mIsBeingCollected && mBoard && !mBoard->mPaused &&
		mApp->mGameScene == GameScenes::SCENE_PLAYING && (IsMoney() || IsSun()))
	{
		PlayCollectSound();
		Collect();
	}
